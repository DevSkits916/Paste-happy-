...
export class QueueWorker {
  constructor({ store, browser, poster, defaults }) { this.store = store; this.browser = browser; this.poster = poster; this.defaults = defaults; this.mode = 'idle'; this.currentJob = null; this.currentStep = 'Idle'; this.lastError = null; this.runPromise = null; }
  ...
  async run() {
    ...
    while (this.mode !== 'stopped' && processed < this.options.maxJobs) {
      ...
      processed += 1; if (this.mode === 'running') await sleep(Math.max(this.options.delayMs, this.options.cooldownMs));
    }
  }
