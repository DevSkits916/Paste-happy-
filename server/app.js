...
export function createApp({ store, worker, browser, root = process.cwd() }) {
  ...
  app.post('/api/queue/start', (req, res, next) => { try { res.status(worker.start(req.body) ? 202 : 409).json(worker.status()); } catch (e) { next(e); } });
  ...
  app.get('/api/status', async (_req, res) => res.json({ ...worker.status(), browser: { profileExists: await browser.exists(), open: Boolean(browser.context), headless: Boolean(browser.headless) } }));
  app.post('/api/browser/login', async (req, res, next) => { try { res.status(202).json(await browser.login({ headless: req.body?.headless })); } catch (e) { next(e); } });
