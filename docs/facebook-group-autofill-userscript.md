...
1. Install a userscript manager such as [Tampermonkey](https://www.tampermonkey.net/) or [Violentmonkey](https://violentmonkey.github.io/) in your browser.
   - On iPhone or iPad, install [Userscripts for Safari](https://apps.apple.com/app/userscripts/id1463298887) from the App Store and enable it under **Settings → Safari → Extensions**.
...
2. Allow the new Facebook tab to load. Within a moment you should see a "Paste Happy" banner indicating the script is preparing your post. On iOS Safari, the banner adapts to safe areas and appears centered near the top of the screen.
...
### Safari on iOS specifics

- When Safari opens the Facebook app-style composer in a new view, the userscript keeps your Copy & Open payload alive until the editor appears, so the text still pastes automatically after the transition.
- The script targets both desktop (`www.facebook.com`) and mobile (`m.facebook.com`/`mobile.facebook.com`) layouts, and taps the first eligible "Write post" entry point if Facebook shows a compact menu.
- If Safari asks for permission to run the Userscripts extension on facebook.com, approve it so the automation can inject the post text.
...
- **Different Facebook layout:** The script tries multiple selectors, including the mobile composer, but Facebook experiments with new UIs. Feel free to tweak the query logic locally if you encounter a new layout.
- **Safari still won't paste:** Confirm the Userscripts extension is allowed on facebook.com under **Settings → Safari → Extensions → Userscripts**. Toggling the extension off and back on can help if Safari stopped injecting scripts.
