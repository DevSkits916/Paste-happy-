...
test('queue API imports CSV and supports get, skip, and retry', async (t) => {
  ...
  const worker = { status: () => ({ state: 'idle' }), start: () => true, pause() {}, resume() {}, stop() {}, clearQueue: async () => ({ cleared: 1 }), skipCurrent: async () => null }; const browser = { exists: async () => false, context: null, headless: false, login: async () => ({ opened: true }) };
  ...
  response = await fetch(`${base}/api/queue/clear`, { method: 'POST' }); assert.deepEqual(await response.json(), { cleared: 1 });
});
