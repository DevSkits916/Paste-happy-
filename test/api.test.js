...
test('queue API imports CSV and supports get, skip, and retry', async (t) => {
  ...
  const worker = { status: () => ({ state: 'idle' }), start: () => true, pause() {}, resume() {}, stop() {}, clearQueue: async () => ({ cleared: 1 }), skipCurrent: async () => null }; const browser = { exists: async () => false, context: null, login: async () => ({ opened: true }) };
